...
st.set_page_config(page_title="Fay Risk Analizi", layout="wide", initial_sidebar_state="collapsed")

# --- ÖZEL CSS (Gereksiz boşlukları silme ve kompakt tasarım) ---
st.markdown("""
    <style>
...
with col_panel:
    ...
    if st.session_state.current_lat and st.session_state.current_lon:
        ...
        # Sonuçları Sol Panele Yazdırma (Kompakt)
        st.markdown("---")
        ...
        st.markdown(f"**📏 Mesafesi:** {distance_km:.2f} km")
        st.markdown(f"**⚙️ Fay Tipi:** {fay_tipi}")
...
with col_map:
    ...
    if faults_display is not None:
        ...
        # Haritayı Sağ Sütuna Bas (use_container_width ile bulunduğu sütuna tam oturur)
        map_output = st_folium(m, use_container_width=True, height=600, key="main_map", returned_objects=["last_clicked"])

        st.caption("ℹ️ **Bilgi:** Renkli çemberler (🔴 1 km | 🟠 5 km | 🟡 15 km) risk alanlarını; mor daireler 5.0 büyüklüğünden büyük geçmiş depremleri gösterir.")
